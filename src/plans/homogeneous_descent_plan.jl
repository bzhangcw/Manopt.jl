@doc raw"""
    HomogeneousDescentModelObjective
...
function HomogeneousDescentModelObjective(
    mho::O, σ::R=1.0
) where {
    E,O<:Union{AbstractManifoldHessianObjective{E},AbstractDecoratedManifoldObjective{E}},R
}
    return HomogeneousDescentModelObjective{E,O,R}(mho, σ)
end
function set_manopt_parameter!(
    f::HomogeneousDescentModelObjective, ::Union{Val{:σ},Val{:RegularizationParameter}}, σ
)
    f.σ = σ
    return f
end

get_objective(arcmo::HomogeneousDescentModelObjective) = arcmo.objective

@doc raw"""
    get_cost(TpM, trmo::HomogeneousDescentModelObjective, X)

Evaluate the tangent space [`HomogeneousDescentModelObjective`](@ref)
...
"""
function get_cost(TpM::TangentSpace, arcmo::HomogeneousDescentModelObjective, X)
    M = base_manifold(TpM)
    p = TpM.point
...
function get_cost_function(arcmo::HomogeneousDescentModelObjective)
    return (TpM, X) -> get_cost(TpM, arcmo, X)
end
@doc raw"""
    get_gradient(TpM, trmo::HomogeneousDescentModelObjective, X)

Evaluate the gradient of the [`HomogeneousDescentModelObjective`](@ref)
...
"""
function get_gradient(TpM::TangentSpace, arcmo::HomogeneousDescentModelObjective, X)
    M = base_manifold(TpM)
    p = TpM.point
    G = get_objective_gradient(M, arcmo, p)
    return G + get_objective_hessian(M, arcmo, p, X) # + arcmo.σ * norm(M, p, X) * X
end
function get_gradient!(TpM::TangentSpace, Y, arcmo::HomogeneousDescentModelObjective, X)
    M = base_manifold(TpM)
    p = TpM.point
...
function get_gradient_function(arcmo::HomogeneousDescentModelObjective)
